...
class SelfieCollectionViewController: UICollectionViewController {
  ...
  override func viewDidAppear(animated: Bool) {
    ...
    if defaults.objectForKey("userLoggedIn") == nil {
      ...
      self.navigationController?.presentViewController(loginController, animated: true, completion: nil)
        ...
        println("NOT LOGINCONTROLLER")
      // check if API token has expired
      let dateFormatter = NSDateFormatter()
        //DELETE THIS LATER BEVERLY

        KeychainAccess.setPassword("2015-04-19 16:28:32 +0000", account: "Auth_Token_Expiry", service: "KeyChainService")
        
        //DELETE THIS LATER BEVERLY
      dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSZ"
        let userTokenExpiryDate : NSString? = KeychainAccess.passwordForAccount("Auth_Token_Expiry", service: "KeyChainService")
        println(KeychainAccess)
        println(KeychainAccess.passwordForAccount("Auth_Token_Expiry", service: "KeyChainService"))

      let dateFromString : NSDate? = dateFormatter.dateFromString(userTokenExpiryDate! as String)
        let now = NSDate()
        println(userTokenExpiryDate!)
        println(NSDate())
  ...
  func clearDataArrayAndReloadCollectionView() {
    // clear data array and reload collection view
  ...
  override func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
    ...
    cell.selfieTitle.text = selfieRowObj.imageTitle as String
    
    var imgURL: NSURL = NSURL(string: selfieRowObj.imageThumbnailURL as String)!
